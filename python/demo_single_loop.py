...
def plot_skeletons(fimg, joints_2d, flag):
    ...
    for ib,bone in enumerate(bone_list):
        ...
        cv2.drawMarker(fimg, (x_0, y_0), color,markerType=cv2.MARKER_SQUARE, markerSize=7)
        cv2.drawMarker(fimg, (x_1, y_1), color,markerType=cv2.MARKER_SQUARE, markerSize=7)
        #print('start',(x_0,y_0))
        #print('end',(x_1,y_1))
    #print('shape', fimg.shape)
    return fimg
...
def save_joints_2px(joints, flag, dimension, filename, output_folder, FOCAL_LENGTH):
    ...
    if flag == 'child':
        if dimension == 3:
            f = open(output_folder + '/alt_results_child'+ str(FOCAL_LENGTH) +'.txt', 'a')
        elif dimension == 2:
            f = open(output_folder + '/alt_results_child2d' + str(FOCAL_LENGTH) + '.txt', 'a')
        line = filename + " " + " ".join(str(r) for v in joints for r in v) + "\n"
        f.write(line)
        f.close()
    elif flag == 'adult':
        if dimension == 3:
            f = open(output_folder + '/alt_results_adult'+ str(FOCAL_LENGTH) +'.txt', 'a')
        elif dimension == 2:
            f = open(output_folder + '/alt_results_adult2d' + str(FOCAL_LENGTH) + '.txt', 'a')
        line = filename + " " + " ".join(str(r) for v in joints for r in v) + "\n"
        f.write(line)
        f.close()

def save_blue(filename, output_folder,flag):
    f = open(output_folder + '/alt_results_blue.txt', 'a')    
    line = filename + ' ' + flag+"\n"
    f.write(line)
    f.close()
...
def save_joints(joints, joints_2d, bboxes, filename, output_folder):
    ...
    f.close()
    """
    f = open(output_folder+'/alt_results_2d.txt','a')
    ...
    if joints_2d.shape[0]!=24:
        for ij in range(0, joints_2d.shape[0]):
            ...
            f.write(line)
    f.close()
    f = open(output_folder+'/alt_bboxes.txt','a')
...
def prepare_dump(pred_results, img, render, bbox_results, FOCAL_LENGTH):
    ...
    focal_new = 1.1*(1e-6 + pred_results['pred_camera'][0][0]*bboxes_size[0])
    focal_new_2 = 1.1*(1e-6 + pred_results['pred_camera'][1][0]*bboxes_size[1]) 
...
def getghost(joints_2d, pred_results):
    bboxes_size = []
    for i in range(0, pred_results['bboxes'].shape[0]):
        msize = max([abs(pred_results['bboxes'][i, 0] - pred_results['bboxes'][i, 2]),abs(pred_results['bboxes'][i, 1] - pred_results['bboxes'][i, 3])])
        bboxes_size.append(msize)
    bbnp = np.asarray(bboxes_size)
    joint_ghost = np.where(bbnp < 100)[0]
    return joint_ghost
...
def parse_args():
    ...
    parser.add_argument('--focal_adult', help='Focal length of the therapist')
    parser.add_argument('--focal_child', help='Focal length of the child')
...
def main():
    ...
    FOCAL_ADULT = float(args.focal_adult)
    FOCAL_CHILD = float(args.focal_child)
    FOCAL_LENGTH = 389
    ...
    with torch.no_grad():
        ...
        outfile = output_folder+ '/alt_results_video.avi'
        
        images = os.listdir(folder_name)
        vid = [s for s in images if 'video_cutted' in s]
        print(vid)
        ...
        while ret:
            joints_NAO=None
            print('nframe', nframe)
            cfg.FOCAL_LENGTH = FOCAL_ADULT
            FOCAL_LENGTH = FOCAL_ADULT
            #389
            ...
            #print('shape_original',img.shape)
            #print('pad', pad_shape)
            ...
            if pred_results is not None:
                ...
                joint_ghost = getghost(joints_2d,pred_results)
                joints_2d = np.delete(joints_2d, joint_ghost,0)
                ...
                if mode_detection == 'blue':
                    ...
                    if rect is None:
                        ...
                        save_blue(file_name, output_folder, 'b')
                        if len(joints_2d) >= 1:
                            ...
                            joints_3d = np.delete(joints_3d, joint_ghost,0)
                            joints2d_adult = joints_2d[0]
                            joints3d_adult = joints_3d[0]
                            flag = 'adult'
                    ...
                    if rect is not None:
                        ...
                        if len(joints_2d) > 1:
                            ...
                            joints_3d = joints_translated.cpu().detach().numpy()
                            joints_3d = np.delete(joints_3d, joint_ghost,0)
                            #print("joints2D",joints_2d)
                            #print("joints3D", joints_3d)
                            #print("joints_NAO",joints_NAO)
                            if joints_NAO is not None:
                                joints_3d = np.delete(joints_3d,joints_NAO,0)
                            joints2d_adult = joints_2d[pose[0]]
                            joints3d_adult = joints_3d[pose[0]]
                            ...
                            save_joints(joints_3d, joints_2d, bbox_real, file_name, output_folder)
                            # if nframe == 71:
                            #     print('passei no dump')
                            #     img_viz = prepare_dump(pred_results, img, render, bbox_results, FOCAL_LENGTH)
                            #     cv2.imwrite(output_folder+'/output_'+flag+'.png', img_viz[:, :, ::-1])
                            FOCAL_LENGTH = FOCAL_CHILD
                            pred_results['pred_translation'] = new_depth(pred_results, FOCAL_LENGTH, param, img)
                            joints_translated = pred_results['pred_joints']+ pred_results['pred_translation'][:,None]
                            #print('Translation: ', pred_results['pred_translation'])
                            joints_3d = joints_translated.cpu().detach().numpy()
                            joints_3d = np.delete(joints_3d, joint_ghost,0)
                            ...
                            save_blue(file_name, output_folder, 'j')
            ret, image = inp.read()
            nframe = nframe + 1
